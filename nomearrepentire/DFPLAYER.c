...
{	
	...
	/* Receive 10 bytes */
	if(!UART_RxBuffer(RxBuffer, sizeof(RxBuffer)))
	{
...
{		
	...
	UART_TxBuffer(TxBuffer, sizeof(TxBuffer));
...
// 	if(RxBuffer[DFP_MSG_IDX_TAIL] != DFP_MSG_VAL_TAIL) return false;
	...
	uint8_t desfazamiento = 3;
	uint8_t cmd = RxBuffer[DFP_MSG_IDX_COMMAND+desfazamiento];
	switch(cmd)
	{
		...
		DFPlayerInfo.FinishedTrack = (RxBuffer[DFP_MSG_IDX_PARAM_MSB+desfazamiento] << 8 | RxBuffer[DFP_MSG_IDX_PARAM_LSB+desfazamiento]);
		break;
		...
		DFPlayerInfo.CurrentStatus = RxBuffer[DFP_MSG_IDX_PARAM_LSB+desfazamiento];
		break;
		case DFP_CMD_GET_CUR_VOLUME:			/* Current volume */
		DFPlayerInfo.CurrentVolume = RxBuffer[DFP_MSG_IDX_PARAM_LSB+desfazamiento];
		break;
		...
		DFPlayerInfo.CurrentTrack = (/*RxBuffer[DFP_MSG_IDX_PARAM_MSB] << 8 |*/ RxBuffer[DFP_MSG_IDX_PARAM_LSB+desfazamiento]);
		break;
...
bool DFP_Alive(){
	return DFP_SendQueryMessage(DFP_CMD_GET_NUM_FILE, 0x00, 0x00);
}